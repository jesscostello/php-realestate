...
$suburbID = mysqli_real_escape_string($link, $_GET['suburbID']);

if (isset($_POST['editSuburb'])) {

$cityID = mysqli_real_escape_string($link, $_POST['cityName']);
$suburb = mysqli_real_escape_string($link, $_POST['suburbName']);

// edit suburb in database
$query = "UPDATE suburbs SET cityID='$cityID', suburbName='$suburb' WHERE suburbID='$suburbID' ";
...
<html>
<head>
    <title>Real Estate Admin - Edit Suburb</title>
	<meta charset="utf-8" />
			...
			<div class="form-group">
					...
					<select name="cityName">
								...
								while ($row = mysqli_fetch_array($result)) {
									extract($row);
								?>
									<option <?php if ($cityID_F == $cityID) echo "selected"; ?> value="<?php echo $cityID; ?>"><?php echo $cityName; ?></option>
								<?php
								} // end of while
								?>
					</select> 
				...
				<div class="form-group">
					<label>Suburb:</label> 
					<input type="text" name="suburbName" size="40" value="<?php echo $suburbName; ?>"/>
				</div>			
			<input type="submit" name="editSuburb" value="Edit This Suburb" class="btn btn-success" />
		</form>
